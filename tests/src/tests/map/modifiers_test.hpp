...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/10/26 19:39:05 by tschmitt          #+#    #+#             */
/*   Updated: 2022/10/26 19:39:07 by tschmitt         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace map
{
	...
	private:
		...
		void _test_erase(container_type &container)
		{
			...
			for (size_t i = 0; i < container.size() / 10; ++i)
			{
				TRY_CATCH_WRITE((*this->ofs) << container.erase(test_case[ i ].first) << ',');
			}
			write::container::to_file(container, *this->ofs);
			(*this->ofs) << std::endl;
	...
	public:
		...
		) : i_base_test<Container>("MapModifiers")
		{
