...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/10/26 19:39:09 by tschmitt          #+#    #+#             */
/*   Updated: 2022/10/26 19:39:10 by tschmitt         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace map
{
	...
	private:
		void _test_find(container_type &container)
		{
			...
			{
				TRY_CATCH_WRITE(
						(*this->ofs) << *container.find(test_case[ i ].first) << ','
				)
			}
			(*this->ofs) << std::endl;
		...
		void _test_count(container_type &container)
		{
			...
			{
				TRY_CATCH_WRITE(
						(*this->ofs) << container.count(test_case[ i ].first) << ','
				)
			}
			(*this->ofs) << std::endl;
		...
		void _test_lower_bound(container_type &container)
		{
			...
			{
				TRY_CATCH_WRITE(
						(*this->ofs) << *container.lower_bound(test_case[ i ].first) << ','
				)
			}
			(*this->ofs) << std::endl;
		...
		void _test_equal_range(container_type &container)
		{
			...
			for (size_t i = 0; i < 10 && i < container.size(); ++i)
			{
				...
				TRY_CATCH_WRITE(
						(*this->ofs) << *container.equal_range(test_case[ i ].first).first << ':'
									 << *container.equal_range(test_case[ i ].first).second << ','
				)
			}
			(*this->ofs) << std::endl;
	...
	public:
		...
		) : i_base_test<Container>("MapOperations")
		{
